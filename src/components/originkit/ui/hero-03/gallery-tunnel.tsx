...
interface ImageBoxProps {
    ...
    grid: number;
    /**
     * Accepted so the responsive value from `useTunnelConfig()` type-checks
     * at the three call sites that already pass it.
     *
     * Deliberately NOT consumed below: the tunnel geometry is fixed by the
     * TUNNEL_WIDTH / TUNNEL_HEIGHT constants, and wiring this in would change
     * how the Cluster Wall and both hero backgrounds render today. Left inert
     * rather than deleted so the responsive breakpoints already computed in
     * use-tunnel-size.tsx stay available when the geometry is made scalable.
     */
    tunnelSize?: number;
    speed: number;
    boost: number;
